...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc , s->dnpc = imm + s->pc, IFDEF(CONFIG_FTRACE, ftrace(s, s->pc, s->dnpc)));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc , s->dnpc = (src1 + imm)& ~1, IFDEF(CONFIG_FTRACE, ftrace(s, s->pc, s->dnpc)));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 00100 11", slliDis, I, /* Nop Instrution */);
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int32_t)src1 >> BITS(imm, 4, 0));
  INSTPAT("0100001 ????? ????? 101 ????? 00100 11", sraiDis, I, /* Nop Instrution */);
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(rd) = src1 & imm);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> BITS(imm, 4, 0));
  INSTPAT("0000001 ????? ????? 101 ????? 00100 11", srliDis, I, /* Nop Instrution */);
